...
class DocumentReader
{
    ...
    public function __construct($filePath,$documentType) 
    {
    ...
    public function convertToText() 
    {
        ...
        if($this->documentType==DOCUMENT_TYPE_DOC) {
    ...
    private function read_doc()	{
        $objWordReader=new MSWordReader($this->filename);
        if(!$objWordReader->isMSWordOld())
        {
            $handlenew=fopen($this->filename, "r");
            fread($handlenew,0x4E00);
            $lines=array();
            $outtext="";
            while(!feof($handlenew))
            {
                $chr=fread($handlenew,1);
                if(ord($chr)==0x0D || ord($chr)==0x0A){}
                else if(ord($chr)<0x20 || ord($chr)>0x7e) continue;
                $outtext=$outtext.$chr;
            }
            fclose($handlenew);
        }
