    ...
    public function getExtraFieldWhere($arrExtraField)
    {
        /*$exWhere="SELECT
                ...
                DATE_FORMAT(DATE_SUB(
                    candidate.date_created, INTERVAL 6 HOUR), '%m-%d-%y'
                ) AS dateCreated,
                DATE_FORMAT(DATE_SUB(
                    candidate.date_modified, INTERVAL 6 HOUR), '%m-%d-%y'
                ) AS dateModified,
DTABLE.VisaStatus
            ...
            LEFT JOIN user AS owner_user
                ...
                LEFT JOIN (select candidate.candidate_id as `excand`,`ex0`.`value` AS `VisaStatus` from candidate LEFT JOIN `extra_field` AS `ex0` ON candidate.candidate_id = ex0.data_item_id  AND ex0.field_name='Visa Status') DTABLE ON candidate.candidate_id=excand
            WHERE
            (
                CONCAT(candidate.first_name, ' ', candidate.last_name) LIKE 'a%'
                OR CONCAT(candidate.last_name, ' ', candidate.first_name) LIKE 'a%'
                OR CONCAT(candidate.last_name, ', ', candidate.first_name) LIKE 'a%'
            )
            AND
...
AND DTABLE.VisaStatus like '%u%'
        ...
        {
            $arrField[]="`STABLE`.`{$field_name}`";
            $arrJoin[]="LEFT JOIN (select {$this->table}.{$this->table}_id as `parent_table_id`,`ext{$ind}`.`value` AS `{$field_name}` from {$this->table} left join `extra_field` AS `ext{$ind}` ON {$this->table}.{$this->table}_id = ext{$ind}.data_item_id  AND ext{$ind}.field_name='{$field_name}') STABLE ON {$this->table}.{$this->table}_id=`parent_table_id`";
        }
        return array("column"=>$arrField,"join"=>$arrJoin);
    }
    ...
    public function buildFilter()
    {
        ...
        foreach($arrWhere as $whr)
        {
            if(is_numeric($whr["field"]))
            {
                ...
                $arrCustomField[]=$fieldName;
                if(empty($where))
                {
                    if($whr["condition"]=="equals")
                        $where="`STABLE`.`{$fieldName}` = '{$whr["data"]}'";
                    else
                        $where="`STABLE`.`{$fieldName}` like '%{$whr["data"]}%'";
                }
                else
                {
                    if($whr["condition"]=="equals")
                        $where=$where." {$whr["boolean"]} `STABLE`.`{$fieldName}` = '{$whr["data"]}'";
                    else
                        $where=$where." {$whr["boolean"]} `STABLE`.`{$fieldName}` like '%{$whr["data"]}%'";
                }
            }
            else
            {
                ...
                {
                    if($whr["condition"]=="equals")
                        $where=$where." {$whr["boolean"]} `{$this->table}`.`{$whr["field"]}` = '{$whr["data"]}'";
                    else
                        $where=$where." {$whr["boolean"]} `{$this->table}`.`{$whr["field"]}` like '%{$whr["data"]}%'";
                }
            }
        }
        $customSqlColum="";
...
public function getExtraFieldWhere($arrExtraField)
    {
        ...
        {
            $arrField[]="`STABLE`.`{$field_name}`";
            $arrJoin[]="LEFT JOIN (select candidate.candidate_id as `parent_table_id`,`ext{$ind}`.`value` AS `{$field_name}` from candidate left join `extra_field` AS `ext{$ind}` ON candidate.candidate_id = ext{$ind}.data_item_id  AND ext{$ind}.field_name='{$field_name}') STABLE ON candidate.candidate_id=`parent_table_id`";
        }
        return array("column"=>$arrField,"join"=>$arrJoin);
    }
public function buildFilter()
    {
        ...
        foreach($arrWhere as $whr)
        {
            ...
            else
            {
                ...
                {
                    if($whr["condition"]=="equals")
                        $where="`candidate`.`{$whr["field"]}` = '{$whr["data"]}'";
                    else
                        $where="`candidate`.`{$whr["field"]}` like '%{$whr["data"]}%'";
                }
                else
                {
                    if($whr["condition"]=="equals")
                        $where=$where." {$whr["boolean"]} `candidate`.`{$whr["field"]}` = '{$whr["data"]}'";
                    else
                        $where=$where." {$whr["boolean"]} `candidate`.`{$whr["field"]}` like '%{$whr["data"]}%'";
                }
            }
        }
        $customSqlColum="";
