...
class imap extends imap_bodystruct {
    ...
    var $state;
    var $read_only;
    var $search_sort_flag;
    
   
    /* set defaults */ 
    function imap() {
        $this->debug = array();
        $this->search_sort_flag = false;
    ...
    function sort_mailbox($mailbox, $sort_type, $filter='ALL') {
        ...
        if ($this->search_sort_flag) {
            return $this->search_sort($mailbox, $sort, $reverse, $filter);
        }
        elseif (($sort == 'THREAD_R' || $sort == 'THREAD_O')) {
            if ($sort == 'THREAD_O') {
    ...
    function client_side_sort($mailbox, $sort, $reverse, $filter, $uid_str=false) {
    ...
    /* UID sequences are not contiguous. This returns a count of valid UIDs within
     * a min and max pair */
    function window_count($uid_str) {
        $cnt = 0;
        $command = 'UID FETCH '.$uid_str." UID\r\n";
        $this->send_command($command);
        $res = $this->get_response(false, true);
        if ($this->check_response($res, true)) {
            $cnt = count($res);
    ...
    /* "windowed" sort to save memory on large folders. Only supports ARRIVAL */
    function search_sort($mailbox, $sort, $reverse, $filter) {
        $max_per_fetch = 400;
        $max_total = 200;
        $count = 0;
        $maxuid = $_SESSION['folders'][$mailbox]['status']['uidnext'];
        $minuid = $maxuid - $max_per_fetch;
        if ($minuid < 1) {
            $minuid = 1;
        }
        $iter = 0;
        $max_total_uid = $maxuid;
        while ($iter < 20 && $minuid > 1 && $count < $max_total) {
            $count += $this->window_count($minuid.':'.$maxuid);
            $maxuid = $minuid;
            $minuid -= $max_per_fetch;
            $iter++;
            if ($minuid < 1) {
                $minuid = 1;
            }
        }
        return $this->client_side_sort($mailbox, $sort, $reverse, $filter, $minuid.':'.$max_total_uid);
    }
