...
function get_config($file) {
    ...
    if (is_array($conf) && !empty($conf)) {
        ...
        if (isset($conf['http_prefix']) && strtolower($conf['http_prefix']) == 'https' && $pre == 'http') {
...
function prep_html_part($string, $uid, $mailbox, $image_replace=false) {
    global $user;
    $replaced = 0;
    ...
    if ($imap->append_start($mailbox, $size)) {
        ...
        if (isset($_SESSION['attachments'][$session]) && !empty($_SESSION['attachments'][$session])) {
            foreach ($_SESSION['attachments'][$session] as $i => $v) {
                ...
                if (is_readable($filename)) {
                    $wsize += $imap->append_feed($headers);
            ...
            $wsize += $imap->append_feed("--".$message->boundry."--");
        }
