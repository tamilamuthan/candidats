...
function message_tags_message_headers_bottom($tools) {
    ...
    if ($uid) {
        ...
        $res .= '<div class="tag_cell message_page"><a id="link_'.$uid.'" onclick="return edit_tags(\''.
                $uid.'\', \''.esc_sq($mailbox).'\');">'.$label.'</a></div>';
    }
...
function message_tags_msglist_after_subject($tools, $msg_vals) {
    ...
    $res = '</div><div style="float: right;"><complex-'.$page_id.'><div class="tag_cell"><a id="link_'.$msg_vals['uid'].'" onclick="return edit_tags(\''.
           $msg_vals['uid'].'\', \''.esc_sq($msg_vals['mailbox']).'\');">'.$label.'</a></div></complex-'.$page_id.'>';
    return $res;
}
