...
function message_tags_init($tools) {
    ...
    if (in_array($tools->get_page(), array('message_tags', 'mailbox', 'new', 'search', 'message')) || ($tools->logged_in() && $tools->get_page() == 'login')) {
        ...
        function edit_cell(tag_str, uid, mbox) {
            var input_cell = "<input id=\"input_"+uid+"\" class=\"tag_input\" onkeypress=\"return check_enter(event, \'"+uid+"\', \'"+esc_sq(mbox)+"\')\"";
            input_cell += " onblur=\"return show_tags(\'"+uid+"\', \'"+esc_sq(mbox)+"\', true);\" type=\"text\" value=\""+esc_dq(tag_str)+"\" />";
            return input_cell;
        }
        ...
        function display_cell(uid, tag_str, mbox) {
            var dsp_cell = "<a id=\"link_"+uid+"\" onclick=\"return edit_tags(\'"+uid+"\', \'"+esc_sq(mbox)+"\');\">";
            if (tag_str === "") {
        ...
        function check_enter(event, uid, mbox) {
            if (event.keyCode == 13 || event.which == 13) { 
                return show_tags(uid, mbox);
            }
            return true;
        }
        function show_tags(uid, mbox, nosave) {
            var input_element = document.getElementById("input_"+uid);
            input_element.onblur = function() {return false;};
        ...
        function edit_tags(uid, mbox) {
            var link = document.getElementById("link_"+uid);
            var tag_str = link.text;
            if (tag_str == undefined) {
                var ih;
                ih = link.innerHTML;
                if (ih.toLowerCase() != "<span class=tag_image title=\"'.strtolower($tools->str[1]).'\"></span>") {
                    tag_str = ih;
                }
            }
            if (tag_str.indexOf("<span") != -1) {
                tag_str = "";
            }
